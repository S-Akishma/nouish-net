...
Be warm, concise, encouraging. Use emojis sparingly.`;
  ...
  try {
    ...
    let reply = "I can help with food donations, requests, delivery tracking, and expiry rules!";
    if (/expir|disappear|gone/.test(m)) reply = "Food listings automatically disappear once their expiry time passes ⏰";
    else if (/nearest|near|match|distance/.test(m)) reply = "Click 'AI Match' on your dashboard to find nearest donors sorted by GPS distance!";
    else if (/donat|add|list|post/.test(m)) reply = "To donate: click '+ Add Food' on your Provider Dashboard.";
    else if (/track|status|deliver/.test(m)) reply = "Track deliveries in 'My Requests' tab. Status: Pending → Accepted → Out for Delivery → Delivered!";
    else if (/hello|hi|hey/.test(m)) reply = "Hello! 👋 I'm NourishBot. I can help you donate food, request food, and track deliveries!";
    res.json({ response: reply, model: 'fallback' });
  }
});

// AI Match — nearest non-expired donors
router.get('/match', auth, async (req, res) => {
  try {
    const userRes = await db.execute({
      sql: 'SELECT lat, lng, name, location_address FROM users WHERE id=?',
      args: [req.user.id]
    });
    ...
    const foodRes = await db.execute({
      sql: `SELECT f.*, u.name as provider_name, u.provider_type,
    ...
    const matched = foodRes.rows.map(f => ({
      ...f,
...
router.get('/live-feed', auth, async (req, res) => {
  try {
    ...
    const receiverRes = await db.execute({
      sql: `SELECT id,name,lat,lng,location_address FROM users WHERE role='orphanage' ORDER BY created_at DESC`,
      args: []
    });
...
// Public nearby suggestion
router.post('/suggest-nearby', async (req, res) => {
  try {
    const { lat, lng } = req.body;
    if (!lat || !lng) return res.json({ message: 'Share your location to see the nearest food donors!' });

    const result = await db.execute({
      sql: `SELECT f.food_name, f.quantity, f.food_type, u.name as provider_name, u.lat as plat, u.lng as plng
            FROM food_listings f JOIN users u ON f.provider_id=u.id
            WHERE f.id NOT IN (
              SELECT food_id FROM requests WHERE status NOT IN ('rejected','delivered')
            )
    ...
    if (sorted.length === 0)
      return res.json({ message: 'No food available near you right now. Check back soon!' });
    ...
    res.json({ message: 'Could not fetch nearby donors.' });
  }
});
